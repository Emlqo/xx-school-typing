...
createRoot(document.getElementById('root')).render(params.has('leaderboard') ? <SubwayLeaderboard
  ...
  records={[
    { id: '1', studentId: 's1', nickname: '완주 학생', subwayStatus: 'completed', subwayProgress: 16, subwayElapsedMs: 65000, subwayPenaltyMs: 8000 },
...
/> : <Game
  ...
  submitRun={async (_, answers) => {
    ...
    window.subwayTestSubmissions = (window.subwayTestSubmissions || 0) + 1;
    return { score: { subwayAnswers: answers, subwayStatus: params.has('expired') ? 'timeout' : answers.length === (params.has('recall') ? 51 : 16) ? 'completed' : 'running', subwayElapsedMs: Date.now() - start } };
  }}
/>);
