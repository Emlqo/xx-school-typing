...
import { enterGameFullscreen, rememberScreenExit, readScreenExit } from '../src/utils/fairPlay.js';
...
function Preview() {
  ...
  const [room] = useState({ mode: 'subway', startedAt: Date.now(), expiresAt: Date.now() + 315000, subway: { practice: 'recall', previewEnabled: true, previewSeconds: 15 } });
  const stop = reason => {
    window.testReports = (window.testReports || 0) + 1;
    rememberScreenExit('test', reason);
    setStage('stopped');
  };
  useGameFocusGuard(stage === 'playing', stop);
  if (stage === 'stopped') return <h1>화면 이탈 · 진행 중단</h1>;
  if (stage === 'entry') return <button onClick={async () => { if (await enterGameFullscreen()) setStage('waiting'); }}>선수 입장</button>;
  if (stage === 'waiting') return <><WaitingView /><button onClick={() => setStage('playing')}>일반 게임 시작</button><button onClick={() => setStage('subway')}>지하철 시작</button></>;
